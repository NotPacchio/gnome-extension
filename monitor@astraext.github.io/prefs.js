...
(function (ColorCategory) {
    ColorCategory[ColorCategory["MAIN"] = 0] = "MAIN";
    ColorCategory[ColorCategory["SECONDARY"] = 1] = "SECONDARY";
})(ColorCategory || (ColorCategory = {}));
export default class AstraMonitorPrefs extends ExtensionPreferences {
    constructor() {
        super(...arguments);
        this.minimumSize = { width: 500, height: 300 };
        this.defaultSize = { width: 800, height: 700 };
        this.expanded = null;
    }
    loadCustomTheme() {
        try {
    ...
    fillPreferencesWindow(window) {
        ...
        if (defaultCategory) {
            if (defaultCategory === 'processors')
                ...
                window.set_visible_page(gpuPage);
    ...
    setupGeneral() {
        ...
        if (!Utils.hasProcStat())
            (check = false),
                this.addStatusLabel({ title: _('Cannot access /proc/stat: this extension will not work!') }, 'am-dialog-error-symbolic', group);
        if (!Utils.hasProcCpuinfo())
            (check = false),
                this.addStatusLabel({ title: _('Cannot access /proc/cpuinfo: this extension will not work!') }, 'am-dialog-error-symbolic', group);
        if (!Utils.hasProcMeminfo())
            (check = false),
                this.addStatusLabel({ title: _('Cannot access /proc/meminfo: this extension will not work!') }, 'am-dialog-error-symbolic', group);
        if (!Utils.hasProcDiskstats())
            (check = false),
                this.addStatusLabel({ title: _('Cannot access /proc/diskstats: this extension will not work!') }, 'am-dialog-error-symbolic', group);
        if (!Utils.hasProcNetDev())
            (check = false),
                this.addStatusLabel({ title: _('Cannot access /proc/net/dev: this extension will not work!') }, 'am-dialog-error-symbolic', group);
        if (!Utils.hasPs())
            (check = false),
                this.addStatusLabel({ title: _("Cannot access 'ps': this extension will not work!") }, 'am-dialog-error-symbolic', group);
        if (!Utils.hasHwmon())
            (check = false),
                this.addStatusLabel({
        ...
        if (!Utils.hasLscpu())
            (check = false),
                this.addStatusLabel({ title: _("'lscpu' not installed: some features will be disabled!") }, 'am-dialog-warning-symbolic', group);
        if (!Utils.hasLspci())
            (check = false),
                this.addStatusLabel({ title: _("'lspci' not installed: some features will be disabled!") }, 'am-dialog-warning-symbolic', group);
        if (!Utils.hasLsblk())
            (check = false),
                this.addStatusLabel({ title: _("'lsblk' not installed: some features will be disabled!") }, 'am-dialog-warning-symbolic', group);
        ...
        if (!Utils.hasIp())
            (check = false),
                this.addStatusLabel({ title: _("'iproute2' not installed: some features will be disabled!") }, 'am-dialog-warning-symbolic', group);
        ...
        this.addSpinRow({
            ...
            icon_name: 'am-dialog-warning-symbolic',
            tabs: 1,
        }, 'headers-height-override', headersSection, { min: 0, max: 80, digits: 0, step: 1, page: 5 }, true, 0);
        this.addFontRow({
    ...
    setupGPU() {
        ...
        this.addSpinRow({ title: _('Update frequency (seconds)') }, 'gpu-update', group, { min: 1, max: 10, digits: 1, step: 0.1, page: 1 }, true, 1.5);
        ...
        this.addColorRow({ title: _('Icon Color'), tabs: 1 }, 'gpu-header-icon-color', iconSection, '');
        this.addColorRow({ title: _('Icon Alert Color'), tabs: 1 }, 'gpu-header-icon-alert-color', iconSection, 'rgba(235, 64, 52, 1)');
        this.addSpinRow({
            title: _('Icon Size'),
            subtitle: _('Experimental feature: may require to disable/enable the extension.'),
            icon_name: 'am-dialog-warning-symbolic',
        ...
        gpuPage.add(group);
        const activityPercentageSection = this.addExpanderRow({ title: _('Activity Percentage') }, group);
        this.addSwitchRow({ title: _('Show Activity Percentage'), tabs: 1 }, 'gpu-header-activity-percentage', activityPercentageSection);
        this.addSpinRow({
            title: _('Icon Alert'),
        ...
        const activityGraphSection = this.addExpanderRow({ title: _('Activity History Graph') }, group);
        this.addSwitchRow({ title: _('Show Activity History Graph'), tabs: 1 }, 'gpu-header-activity-graph', activityGraphSection);
        this.addColorRow({
            title: _('Main Color'),
            subtitle: _('<b>Total usage</b> color.'),
            tabs: 1,
        }, 'gpu-header-activity-graph-color1', activityGraphSection, 'rgba(29,172,214,1.0)');
        this.addSpinRow({ title: _('History Graph Width'), tabs: 1 }, 'gpu-header-activity-graph-width', activityGraphSection, { min: 10, max: 500, digits: 0, step: 1, page: 10 }, true, 30);
        const activityBarsSection = this.addExpanderRow({ title: _('Realtime Activity Bar') }, group);
        this.addSwitchRow({ title: _('Show Activity Bar'), tabs: 1 }, 'gpu-header-activity-bar', activityBarsSection);
        this.addColorRow({
            title: _('Bar Color'),
            subtitle: _('GPU bar main color.'),
            tabs: 1,
        }, 'gpu-header-activity-bar-color1', activityBarsSection, 'rgba(29,172,214,1.0)');
        const memoryPercentageSection = this.addExpanderRow({ title: _('Memory Percentage') }, group);
        this.addSwitchRow({ title: _('Show Memory Percentage'), tabs: 1 }, 'gpu-header-memory-percentage', memoryPercentageSection);
        this.addSpinRow({
            title: _('Icon Alert'),
        ...
        this.addSwitchRow({ title: _('Show Memory History Graph'), tabs: 1 }, 'gpu-header-memory-graph', memoryGraphSection);
        this.addColorRow({
            title: _('Main Color'),
            subtitle: _('<b>Total usage</b> color.'),
            tabs: 1,
        }, 'gpu-header-memory-graph-color1', memoryGraphSection, 'rgba(29,172,214,1.0)');
        this.addSpinRow({ title: _('History Graph Width'), tabs: 1 }, 'gpu-header-memory-graph-width', memoryGraphSection, { min: 10, max: 500, digits: 0, step: 1, page: 10 }, true, 30);
        const memoryBarsSection = this.addExpanderRow({ title: _('Realtime Memory Bar') }, group);
        this.addSwitchRow({ title: _('Show Memory Bar'), tabs: 1 }, 'gpu-header-memory-bar', memoryBarsSection);
        this.addColorRow({
            title: _('Bar Color'),
            subtitle: _('GPU bar main color.'),
            tabs: 1,
        ...
        this.addSwitchRow({ title: _('Activity Percentage'), tabs: 0 }, 'gpu-header-tooltip-activity-percentage', group);
        this.addSwitchRow({ title: _('Memory Percentage'), tabs: 0 }, 'gpu-header-tooltip-memory-percentage', group);
        this.addSwitchRow({ title: _('Memory Value'), tabs: 0 }, 'gpu-header-tooltip-memory-value', group);
        gpuPage.add(group);
        return gpuPage;
    }
    setupMemory() {
    ...
    setupAbout(window) {
        ...
        this.addLinkRow({ title: _('Roadmap') }, 'https://github.com/AstraExt/astra-monitor/blob/main/ROADMAP.md', group);
    ...
    addTextInputRow(props, setting, group, reset) {
        const tabs = props.tabs;
        delete props.tabs;
        const row = new Adw.ActionRow(props);
        if (tabs)
    ...
    addButtonRow(props, group, callback) {
        const tabs = props.tabs;
        delete props.tabs;
        const row = new Adw.ActionRow(props);
        if (tabs)
    ...
    addStatusLabel(props, iconName, group) {
        const tabs = props.tabs;
        delete props.tabs;
        const row = new Adw.ActionRow(props);
    ...
    addSwitchRow(props, setting, group) {
        const tabs = props.tabs;
        delete props.tabs;
        if (props.icon_name) {
            if (props.title)
    ...
    addColorRow(props, setting, group, reset) {
        const tabs = props.tabs;
        delete props.tabs;
        if (props.icon_name) {
            if (props.title)
    ...
    addDropRow(props, values, setting, group, type = 'int', reset) {
        const tabs = props.tabs;
        delete props.tabs;
        let selected = -1;
        let savedValue;
    ...
    addComboRow(props, values, setting, group, type = 'int', reset) {
        const tabs = props.tabs;
        delete props.tabs;
        let selected = -1;
        let savedValue;
    ...
    addSpinRow(props, setting, group, adj, numeric = true, reset) {
        const tabs = props.tabs;
        delete props.tabs;
        if (props.icon_name) {
            if (props.title)
    ...
    addFontRow(props, setting, group, reset) {
        if (!Utils.metadata)
            throw new Error('Metadata not found');
        const tabs = props.tabs;
        delete props.tabs;
    ...
    addOrderingRows(config, group, tabs = 0) {
    ...
    OrderingItem(item, index, count, config, group, tabs = 0) {
